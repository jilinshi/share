...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String _title = "殡葬数据查询";
	String _pagetype="PpBurial";
%>
<title>核对结果导出</title>
		...
		<div class="widget-box">
			...
			<div class="form-group">
				...
				<button type="button" class="btn btn-info btn-sm"  onclick="javascript:onClik();">
					...
					<br><br>
					<a href="<%=basePath%>downloadExcel.action?fileName=ppburialxls1" class="btn btn btn-sm">
						<i class="ace-icon fa fa-file-excel-o bigger-90"></i>按Excel导出核对数据
					</a>
					&nbsp;
					<a href="<%=basePath%>downloadCSV.action?fileName=ppburialcsv1" class="btn btn btn-sm">
						<i class="ace-icon fa fa-file-excel-o bigger-90"></i>按CSV导出核对数据
					</a>
					&nbsp;
					<a href="<%=basePath%>downloadExcel.action?fileName=ppburial" class="btn btn btn-sm">
						<i class="ace-icon fa fa-file-excel-o bigger-90"></i>按Excel导出核对结果
					</a>
					&nbsp;
					<a href="<%=basePath%>downloadCSV.action?fileName=ppburialcsv" class="btn btn btn-sm">
	...
	$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
	   ...
	   $(window).triggerHandler('resize.jqGrid');//trigger window resize to make the grid get the correct size
		...
		jQuery(grid_selector).jqGrid({
			...
			height: 450,
		    colNames: ['','家庭编号','低保成员姓名','低保身份证号码','区','街道','社区','姓名_殡葬','身份证_殡葬','火化时间'],
			colModel:[
